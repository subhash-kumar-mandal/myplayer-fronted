...
const AlbumCardBigLeft = ({ info }) => {
    ...
    return (
        ...
        "
            >
                <div className="h-[133.725px] w-[133.725px]  rounded-[5px] overflow-hidden shadow-[0_8px_20px_rgba(0,0,0,.35c)]">
                    <img src={info.image.url} className='w-full h-full object-cover' alt="" />
                </div>

                <div className="flex flex-col">
                    <p className="text-[14px] hover:underline capitalize text-wrap cursor-pointer"
                        onClick={(e) => {
                            navi(`/album/${info._id}`)
                            e.stopPropagation()
                        }}
...
export default AlbumCardBigLeft