        ...
        "
            >
                <div className="h-[153.725px] w-[153.725px]  rounded-[5px] overflow-hidden shadow-[0_8px_20px_rgba(0,0,0,.35c)]">
                    <img src={single.image.url} className='w-full h-full object-cover' alt="" />
                </div>

                <div className="flex flex-col">
                    <p className="text-[16px] hover:underline capitalize cursor-pointer"
                     onClick={(e)=>{
                        navi(`/track/${single._id}`)
                        e.stopPropagation()
                     }}
                    >
                        {
                            single.name.length > 15 ? single.name.slice(0, 14) + "..." : single.name
                        }
                    </p>
